...
public abstract class AbstractUpgradeScreen<T extends AbstractUpgradeScreenHandler> extends HandledScreen<T> {
    ...
    private static final Identifier TEXTURE = new Identifier(NepIO.MODID, "textures/gui/container/upgrade_screen.png");
    ...
    protected void drawTab(MatrixStack matrices) {
    ...
    protected void drawGuiSlot(MatrixStack matrices, Inventory inventory) {
        ...
        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
        RenderSystem.setShaderTexture(0, TEXTURE);
        int x = (int) (backgroundWidth/2-(float) inventory.size()/2.0F*18.0F)+(width-backgroundWidth)/2;
        int y = 34+(height-backgroundHeight)/2;
        for (int i = 0; i < inventory.size(); ++i) {
            drawTexture(matrices, x, y, 176, 0, 18, 18);
            x += 18;
        }
    }
